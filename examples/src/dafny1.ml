...
let analyze_rels_among_pfs pfs_lists =
  let rec wrapper pfs_lists res =
    ...
    | pfs_list::pfs_lists' ->
      ...
      (**find the same type name paramref list*)
        let related =
          List.filter (List.concat pfs_lists') ~f:(fun (Paramfix(_, tn', _)) -> tn = tn')
        in
        (**find the same consts paramref list in the related list*)
...
let get_pf_name_type_list pfs =
  ...
  (* part1:0<= i<N0  part2:p__Inv0!=p__Inv2&&p__Inv2<N0&& p__Inv0<N0 *)
...
let inv_act cinv =
  ...
  let inv_type = pd_str  in
  let varNames =(let ()=VarNamesWithParam.of_var_ref :=  (fun _x -> String.Set.of_list [var_act _x]) in
  VarNamesWithParam.of_form  gened_inv'' ~types:[])   in
  ...
  let varNamesOfInv =(*let s'=Trans.trans_statement ~types s in  VarNames.of_statement s' in*)
          (let ()=VarNamesWithParam.of_var_ref :=  (fun _x -> String.Set.of_list [ToDafny.var_act _x]) in
           VarNamesWithParam.of_form (Trans.invTrans_formula gened_inv) ~types:[])   in
  let varNames1 =remove_duplicates (List.concat (List.map ~f:ToDafny.getallInvnames (String.Set.to_list varNamesOfInv))) in        
  let	varNamesOfInv_str=String.concat ~sep:"," ( varNames1) in
  let param_constr_str2 = String.concat ~sep:"\n" 
           (List.map ~f:(fun x-> if (Hashtbl.mem ToDafny.vartable x) then (sprintf "requires %s.Length==N0" (x)) else (sprintf ""))  varNames1) in 
  ...
  %s 
  %s
  {\n%s\n%s\n}\n" name pds_str1 (if pd_str="" then "" else (","^pd_str)) allReadConstr paramConstrOnNs param_constr_str2  (form_act_in_murphi (neg gened_inv'')) varNamesOfInv_str
...
let gen_case_1 indent cut_tacs ="//case 1"


let gen_case_2 indent cut_tacs invStr=sprintf "requires %s //case 2" invStr


let gen_case_3 indent cut_tacs (ConcreteProp(Prop(_, _, f), _)) =
	sprintf "requires %s//case 3" (ToDafny.form_act (Trans.invTrans_formula(Paramecium.neg f)))


let gen_branch branch case =
  sprintf "requires %s//branch\n%s" branch case 
...
let file_pt name types_str vardefStr  predicates_str () = 
    let pub_str = sprintf
"%s\n%s\n%s\n"  types_str vardefStr predicates_str   in
  Out_channel.write_all (sprintf "%s/%s_predicates.dfy" name name) pub_str;;
...
source ~/.bashrc
%s
"(*gen_cmd (sprintf "%s_base.dfy" name)*) (String.concat ~sep:"\n" (
...
let protocol_act' protocol cinvs_with_varnames relations=
  ...
  file_pub name types_str ();
  file_inv types types_str record_var_str  name relations rules () ;
  file_sh name (List.length invs) () ;
  (* let ()=file_pt name  types_str record_var_str predicates_str () in  *)
...
let protocol_act' protocol cinvs_with_varnames relations=
 ...
 (* let ()=print_endline (sprintf "%d\n" (Hashtbl.length ToDafny.var2SymbNoTab)) in
  ...
  String.concat ~sep:"\n\n\n" [types_str; record_var_str; ""; predicates_str]   
