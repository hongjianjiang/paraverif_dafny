...
ensures   (!((top.AChan3_1.Cmd == InvAck) && (top.CurCmd == Empty)))
...
requires (!((top.AChan3_1.Cmd == InvAck) && (top.CurCmd == ReqS) && (top.ExGntd == false)))//3//guard condition
...
requires 
requires (!((top.AChan3_1.Cmd == InvAck) && (top.CurCmd == Empty)))//statement has nothing with prop--it guranttee itself
//guard condition
...
method n_n_Store_i1inv__86_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
...
method n_n_AStore_i1inv__86_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
