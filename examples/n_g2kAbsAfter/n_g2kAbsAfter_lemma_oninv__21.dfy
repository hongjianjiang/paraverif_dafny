...
requires (!((top.ACache_1.State == E) && (top.Chan2_1.Cmd == Inv)))//3//guard condition
requires   ((top.Chan2_1.Cmd == Inv) && (top.Chan3_1.Cmd == Empty));
ensures   (!((top.Chan3_1.Cmd == InvAck) && (top.ACache_1.State == E)))
...
requires (!((top.Chan3_1.Cmd == InvAck) && (top.AChan2_1.Cmd == GntE)))//3//guard condition
...
requires 
requires (!((top.Chan3_1.Cmd == InvAck) && (top.ACache_1.State == E)))//statement has nothing with prop--it guranttee itself
//guard condition
...
method n_n_Store_i1inv__21_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
...
method n_n_AStore_i1inv__21_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
