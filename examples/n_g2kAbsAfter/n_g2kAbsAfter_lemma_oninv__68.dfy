...
ensures   (!((!(top.Cache_1.State == I)) && (top.Chan3_1.Cmd == InvAck)))
...
requires (!((top.Chan2_1.Cmd == GntS) && (top.Chan3_1.Cmd == InvAck)))//3//guard condition
...
requires (!((top.Chan3_1.Cmd == InvAck) && (top.Chan2_1.Cmd == GntE)))//3//guard condition
...
requires 
requires (!((!(top.Cache_1.State == I)) && (top.Chan3_1.Cmd == InvAck)))//statement has nothing with prop--it guranttee itself
//guard condition
...
method n_n_Store_i1inv__68_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
...
method n_n_AStore_i1inv__68_0(top:TopC,d:nat, N4:nat )
requires 0<= d<N4
