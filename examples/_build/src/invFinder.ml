...
let relation_2_str relation =
  ...
  | InvHoldForRule3(cp) -> 
    ...
    Prt.warning(sprintf "invHoldForRule3-%s:%s" pname (ToStr.Smv.form_act form));
...
module FalseInvLib=struct
	...
	let add inv=
		 match List.find (!falseInvs) ~f:(fun p -> symmetry_form p inv = 0) with
    | Some(cinv) -> cinv
    | None ->
      (*let cinv = form_2_concreate_prop ~id:(!index) inv in*)
      print_endline ("print False Inv");
      print_endline (ToStr.Smv.form_act ( inv));
      incr index;
      falseInvs := ((!falseInvs)@[ inv]);
      inv
  ...
  let mem inv=    
	...
	let rec inter invs=
	 match invs with
	 |[] -> false
	 |x::xs -> if (mem x ) then true else inter xs
end

let rec trySimpleSymList fs=
	match fs with
	|[] -> true
	|f::xs -> 
		begin
...
module Choose = struct
  ...
  let check_level ?(must_new=false) inv =
    ...
    if is_tautology (neg inv) then
      ...
      with
      | _ -> (*let tmp=FalseInvLib.add inv in*) not_inv
    end
  ...
  let gen_of_a_type inv_pds rule_pds =
    ...
    | (Paramdef(_, _))::_ ->
      ...
      List.map semi_consts ~f:(fun ls ->
        List.map2_exn ls rule_pds ~f:(fun c (Paramdef(name, tname)) -> paramfix name tname c)
      )
...
module RenameParam = struct
  ...
  and form_act ?(pfs_rule=(!pfs_rule_ref)) ?(pfs_prop=(!pfs_prop_ref)) f =
    ...
    | ForallFormula(pds,f) -> forallFormula pds (form_act f)
    | ExistFormula(pds,f) -> existFormula pds (form_act f)
end
...
let deal_with_case_3 crule cinv cons g =
  ...
  let (new_inv, causal_cinv) =
    ...
    | Choose.New_inv(inv) ->
      ...
      Prt.warning (sprintf "rule %s, new %s, old %s" name new_inv_str old_inv_str);
    ...
    | Choose.Not_inv ->
      ...
      let tmp=FalseInvLib.add (concrete_prop_2_form cinv) in
...
let tabular_expans (Rule(_name, _, form, _), crule, _, assigns) ~cinv =
  ...
  let ConcreteProp(property, pfs_prop) = cinv in
...
let rec tabular_rules_cinvs rname_paraminfo_pairs cinvs relations =
  ...
  |Empty_exception->
    ...
    |> List.map ~f:(fun cinv ->
  	...
  	print_endline (String.concat ~sep:"\n" falseinvs_strs);
  	InvLib.pairs:=[];
  	tabular_rules_cinvs rname_paraminfo_pairs cinvs relations
...
let find ?(insym_types=[]) ?(smv_escape=(fun inv_str -> inv_str))
  ...
  let init_cinvs =
    let invs =
      List.concat (List.map properties ~f:(fun x->let ()=print_endline (let Prop(pn,pds,pf)=x in pn) in simplify_prop x))
