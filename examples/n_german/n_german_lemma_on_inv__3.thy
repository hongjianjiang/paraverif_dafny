...
have "(True)\<or>(True)\<or>(True)\<or>(True)\<or>(True)\<or>(True)" apply (cut_tac a1 a2, auto) done
moreover {
  assume b1: "(True)"
  have "?P3 s"
  apply (cut_tac a1 a2 b1, simp, rule_tac x="(neg (andForm (eqn (IVar (Field (Para (Ident ''Cache'') i) ''State'')) (Const E)) (eqn (IVar (Ident ''ExGntd'')) (Const false))))" in exI, auto) done
...
have "(True)\<or>(True)\<or>(True)" apply (cut_tac a1 a2, auto) done
moreover {
  assume b1: "(True)"
  ...
  moreover {
    ...
    have "?P3 s"
    apply (cut_tac a1 a2 b1 c1, simp, rule_tac x="(neg (andForm (andForm (eqn (IVar (Field (Para (Ident ''Chan3'') i) ''Cmd'')) (Const InvAck)) (eqn (IVar (Ident ''ExGntd'')) (Const true))) (neg (eqn (IVar (Field (Para (Ident ''Chan3'') i) ''Data'')) (IVar (Ident ''AuxData''))))))" in exI, auto) done
    then have "invHoldForRule s f r (invariants N)" by auto
  }
  moreover {
